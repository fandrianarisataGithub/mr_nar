...
class ClientController extends AbstractController
{
    ...
    /*public function register_client(Client $client = null, Request $request, ClientRepository $repoClient, UserRepository $repos, EntityManagerInterface $manager,SessionInterface $session)
    {
        ...
        // pour l'affichage des form pour image lors des modif
        $misy = "oui";
        ...
        if(!$client){
            $misy = "non";
            $client = new Client();
            $form_register = $this->createFormBuilder($client)
       ...
       // dd($clients_du_jour);
        if($form_register->isSubmitted() && $form_register->isValid()){
            ...
            if(!$client){
                ...
                $client->setImage1($image_cin_1);
                $client->setImage2($image_cin_2);
                ...
                $client->setImage1($client->getImage1());
                $client->setImage2($client->getImage2());
                dd($client->getImage1());
                $client->setTabPointage($client->getTabPointage());
            }
